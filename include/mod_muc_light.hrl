...
-type aff_user() :: {jlib:simple_bare_jid(), aff()}.
-type aff_users() :: [aff_user()].
-type validation_error() :: {error, {Key :: binary(), Reason :: atom()}}.

-type external_check_fun() :: fun((RoomUS :: jlib:simple_bare_jid(),
...
-type blocking_who() :: jlib:simple_bare_jid().
-type blocking_item() :: {
        What :: blocking_what(),
...
-type disco_room_info() :: {RoomUS :: jlib:simple_bare_jid(),
                            RoomName :: binary(),
                            RoomVersion :: binary()}.
