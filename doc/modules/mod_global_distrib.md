...
* Domains of non-hidden components and services (see the [`XMPP Components`](../listeners/listen-components.md) documentation) are stored in per-node set structures where the key is `<local_host>#<node_name>#{public_domains}`, and the values are the domain names.
