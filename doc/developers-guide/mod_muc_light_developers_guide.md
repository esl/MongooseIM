...
* `mod_muc_light_codec.erl`
  ...
  Translates the MUC Light internal data format to stanzas for clients and vice versa.
  It implements the modern MUC Light protocol described in the XEP, supporting all MUC Light features.
...
There is one test suite and one helper module in `big_tests/tests`.
...
Adds a MUC service item to the Disco result, using the MUC Light namespace.
...
* `filter_room_packet` by the codec
