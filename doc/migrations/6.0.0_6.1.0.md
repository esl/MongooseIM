...
* You might be interested in the new C2S listener options: `max_connections`, `c2s_state_timeout`, `reuse_port` and [`backwards_compatible_session`](../listeners/listen-c2s.md#listenc2sbackwards_compatible_session). The first two options can be set for websockets as well.
