...
* [Listeners](../configuration/listen.md). Currently, it only affects [`http`](../listeners/listen-http.md#http-based-services-listenhttp) and [`c2s`](../listeners/listen-c2s.md#client-to-server-c2s-listenc2s) with `tls.module` set to `"just_tls"`, but we recommend fixing it for all listeners already, because in future releases all listeners would have this validation.
