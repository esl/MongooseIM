...
This logic is unchanged, but there was an issue with the default configuration file - it used an access rule `c2s_shaper` instead of the [`normal` shaper](../configuration/shaper.md#c2s-shaper).
This is fixed in the new default configuration, but you might need to update it in your configuration as well.
It is recommended to remove the related `c2s_shaper` access rule as well to avoid confusion.
...
Previously, this option referenced the access rule name instead.
In the default configuration file, the `s2s_shaper` rule was replaced by the corresponding [`fast` shaper](../configuration/shaper.md#s2s-shaper).
You might need to apply this fix in your configuration file.
It is recommended to remove the related `s2s_shaper` access rule as well to avoid confusion.
Keep in mind that you might have several listeners configured.
...
Options from the [s2s](../configuration/s2s.md) section have been reworked. All affected options are listed below:
...
* `s2s.max_retry_delay` was renamed to [`s2s_outgoing.max_retry_delay`](../configuration/s2s.md#s2soutgoingmax_retry_delay).
...
The component listener section used to be called `listen.service`, and it was renamed to [`listen.component`](../listeners/listen-components.md).
...
| `c2s_xmpp_element_size_in_byte_size`, `c2s_xmpp_element_out_byte_size`,<br> `s2s_xmpp_element_size_in_byte_size`, `s2s_xmpp_element_out_byte_size`,<br> `component_xmpp_element_size_in_byte_size`, `component_xmpp_element_out_byte_size` | `xmpp_element_in_byte_size`, `xmpp_element_out_byte_size` | [XMPP traffic](../operation-and-maintenance/MongooseIM-metrics.md#xmpp-traffic-metrics) |
