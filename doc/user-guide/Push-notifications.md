...
In that case you need to create a plugin module that implements the `mod_push_plugin` behaviour and enable this plugin
as specified in the [mod_push][] documentation.

#### [mod_pubsub][] with [mod_push_service_mongoosepush][] a.k.a. '_App Server_'

The next component to configure consist of two modules:

  * [mod_pubsub][] with a `push` node type enabled that will act as a sink for push notifications generated by [mod_push][]
