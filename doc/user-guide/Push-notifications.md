...
You can also control the format of "sender" of push notification (which ultimately becomes the title of push
notification) and filter which messages will trigger the notification. In that case you need to create a plugin
module that implements the `mod_push_plugin` behaviour and enable this plugin as specified in [mod_push][]
documentation.
