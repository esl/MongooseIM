...
* `modules` (list of tuples: `{Host, Path, Modules}`) - List of enabled HTTP-based modules. `"_"` equals any host.
  ...
  * `mod_metrics` - REST API for accessing internal MongooseIM metrics. Please refer to [[REST interface to folsom metrics]] for more information. Default declaration: `{"localhost", "/metrics", mod_metrics}`.

## ejabberd_s2s_in

Handles incoming S2S connections. Relies on `ejabberd_listener` and `ejabberd_receiver` just like `ejabberd_c2s`.

**Note:** Many S2S options are configured as top-level config options and they apply to both incoming and outgoing connections. Please refer to [[Advanced configuration]] for more information.
