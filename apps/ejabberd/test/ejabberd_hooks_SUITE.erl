...
hooks_run_launches_nullary_fun(_) ->
    given_hooks_started(),
    given_module(hook_mod, fun_nullary, fun(_) -> success0 end),
    given_hook_added(test_run_hook, hook_mod, fun_nullary, 1),
    ...
    [{_,{hook_mod,fun_nullary,[_]}, success0}] = H.

hooks_run_launches_unary_fun(_) ->
    given_hooks_started(),
    given_module(hook_mod, fun_onearg, fun(_Acc, Val) -> Val end),
    ...
    %% then
    [{_,{hook_mod,fun_onearg,[_, oneval]}, oneval}] = meck:history(hook_mod).

hooks_run_ignores_different_arity_funs(_) ->
    given_hooks_started(),
    given_module(hook_mod, fun_onearg, fun(_, unused) -> never_return end),
    given_fun(hook_mod, fun_twoarg, fun(_, one,two)-> success2 end),
    ...
    %% then
    [{_,{hook_mod,fun_twoarg,[_, one,two]}, success2}] = meck:history(hook_mod).
...
hooks_run_stops_when_fun_returns_stop(_) ->
    ...
    %% then
    [{_,{hook_mod,a_fun,[_]}, stop}] = meck:history(hook_mod).
