...
%%==============================================================================
-module(mod_mam_riak_bucket_arch).
...
archive_message(_, _, MessID, _ArchiveID, LocJID, RemJID, SrcJID, Dir, Packet) ->
    ...
    Key = key(RemoteJID, MsgId),
    Obj = riakc_obj:new(bucket(LocalJID), Key, encode_riak_obj(SourceJID, Packet)),
...
bucket_key_filters(LocalJid, MsgId) when is_integer(MsgId) ->
    ...
    {bucket(LocalJid), EndsWith};
bucket_key_filters(LocalJid, RemoteJid) ->
    {bucket(LocalJid), key_filters(RemoteJid)}.

bucket_key_filters(LocalJid, RemoteJid, undefined, undefined) ->
    bucket_key_filters(LocalJid, RemoteJid);
bucket_key_filters(LocalJid, RemoteJid, Start, End) ->
    {bucket(LocalJid), key_filters(RemoteJid, Start, End)}.

key_filters(undefined) ->
    [];
key_filters(RemoteJid) ->
    RemoteJidBin = ?BARE_JID(RemoteJid),
    [[<<"starts_with">>, <<RemoteJidBin/binary>>]].

key_filters(RemoteJid, Start, End) ->
    JidFilter = key_filters(RemoteJid),
