...
aws_handle(Host) ->
    ...
    SecretKey = [_ | _] =  opt(Host, secret_access_key),
    SNSHost = [_ | _] =  opt(Host, sns_host),
...
opt(Host, Option) ->
    gen_mod:get_module_opt(Host, ?MODULE, Option, undefined).
...
user_guid(Host, UserJID) ->
    PluginModule = opt(Host, plugin_module),
