...
-record(mam_muc_report_state, {action, from_jid, arc_jid, iq}).
...
-type report_state() :: #mam_muc_report_state{
        action :: mam_action(),
...
-spec handle_get_prefs(ejabberd:jid(), report_state(), ejabberd:iq()) ->
        ejabberd:iq().
...
-spec handle_lookup_messages(From :: ejabberd:jid(), ArcJID :: ejabberd:jid(),
        RS :: report_state(), IQ :: ejabberd:iq()) -> ejabberd:iq().
...
-spec report_error(ReportState :: report_state(), Desc :: string(),
                   Reason :: term()) -> ok.
report_error(ReportState=#mam_muc_report_state{}, Desc, Reason) ->
    #mam_muc_report_state{
        action=Action,
        from_jid=From,
        arc_jid=ArcJID,
