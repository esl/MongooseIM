...
-type hook() :: [atom() | jlib:server() | integer(), ...].
-type t() :: hook().
...
-spec sm_register_connection_hook(map(), tuple(), jlib:jid(), term()
...
-spec sm_remove_connection_hook(map(), tuple(), jlib:jid(),
...
-spec user_send_packet(map(), jlib:jid(), tuple(), tuple()
                      ) -> metrics_notify_return().
...
-spec user_send_packet_type(Server :: jlib:server(),
...
-spec user_receive_packet(map(), jlib:jid(), tuple(), tuple(), tuple()) -> term().
...
-spec user_receive_packet_type(Server :: jlib:server(),
                               Packet :: exml:element()) -> ok | {error, not_found}.
...
-spec xmpp_stanza_dropped(mongoose_acc:t(), jlib:jid(), jlib:jid(), exml:element()) ->
    metrics_notify_return().
xmpp_stanza_dropped(Acc, #jid{server = Server} , _, _) ->
    mongoose_metrics:update(Server, xmppStanzaDropped, 1),
    Acc.

-spec xmpp_send_element(Acc :: map(), Server :: jlib:server()) -> ok | metrics_notify_return().
...
-spec roster_get(list(), {_, jlib:server()}) -> list().
roster_get(Acc, {_, Server}) ->
    mongoose_metrics:update(Server, modRosterGets, 1),
    Acc.

-spec roster_set(Acc :: map(), JID :: jlib:jid(), tuple(), tuple()) ->
    metrics_notify_return().
...
-spec roster_push(map(), jlib:jid(), term()) -> metrics_notify_return().
...
%% #rh
-spec register_user(map(), binary(), jlib:server()) -> metrics_notify_return().
register_user(Acc, _, Server) ->
    mongoose_metrics:update(Server, modRegisterCount, 1),
    Acc.

-spec remove_user(map(), binary(), jlib:server()) -> metrics_notify_return().
...
-spec privacy_iq_get(term(), jlib:jid(), jlib:jid(), term(), term()) -> term().
