...
-type content_source() :: message | jingle.
-type condition_map() :: #{event => msg | unack_msg,
                           type => chat | groupchat,
                           body => absent | empty | non_empty,
                           hint => no_store | store,
                           jingle => boolean(),
                           user_status => online | offline,
                           client_state => active | inactive}.
-type conditions() :: [condition_map()].
-type rule() :: #{conditions := conditions(),
                  action := push | skip,
                  content => content_source()}.
-type condition_key() :: atom().
-type condition_value() :: atom().

-spec config_spec() -> mongoose_config_spec:config_list().
config_spec() ->
    #list{items = rule_config_spec(), validate = unique}.

-spec push_event(mongoose_acc:t(), event(), jid:jid(), [rule()]) ->
          mongoose_acc:t().
push_event(Acc, Event, BareRecipient, Rules) ->
    perform_action(Acc, Event, BareRecipient, match_rule(Rules, Event)).

-spec perform_action(mongoose_acc:t(), event(), jid:jid(), rule()) -> mongoose_acc:t().
...
rule_config_spec() ->
    #section{items = #{~"conditions" => conditions_config_spec(),
             ...
             defaults = #{~"conditions" => [#{}]}, % always matches
             required = [~"action"],
...
-spec match_rule([rule()], event()) -> rule().
...
-spec check_conditions(conditions(), event()) -> boolean().
check_conditions(Conditions, Event) ->
    lists:any(fun(ConditionMap) -> check_condition_map(ConditionMap, Event) end, Conditions).

-spec check_condition_map(condition_map(), event()) -> boolean().
check_condition_map(ConditionMap, Event) ->
    lists:all(fun({Key, Value}) -> check_condition(Key, Value, Event) end,
              maps:to_list(ConditionMap)).

-spec check_condition(condition_key(), condition_value(), event()) -> boolean().
