...
-callback push_event(Host :: jlib:lserver(), Event :: event()) -> any().
...
-spec push_event(mongoose_acc:t(), Host :: jlib:server(), Event :: event()) -> mongoose_acc:t().
...
-spec ets_name(Host :: jlib:server()) -> atom().
ets_name(Host) ->
    gen_mod:get_module_proc(Host, ?MODULE).

-spec create_ets(Host :: jlib:server()) -> any().
create_ets(Host) ->
    Self = self(),
