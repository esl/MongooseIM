...
-callback user_guid(UserJID :: jlib:jid()) -> user_guid().
-callback message_attributes(TopicARN :: topic_arn(), UserJID :: jlib:jid(),
                             IsOnline :: boolean()) -> attributes().
-callback message_attributes(TopicARN :: topic_arn(), From :: jlib:jid(),
                             To :: jlib:jid(), MessageType :: pm | muc,
...
-spec start(Host :: jlib:server(), Opts :: proplists:proplist()) -> ok.
start(Host, Opts) ->
...
-spec user_presence_changed(UserJID :: jlib:jid(), IsOnline :: boolean()) -> ok.
...
-spec handle_packet(From :: jlib:jid(), To :: jlib:jid(),
...
-spec async_publish(Host :: jlib:lserver(), topic_arn(), Content :: jiffy:json_value(),
              attributes()) -> ok.
async_publish(Host, TopicARN, Content, Attributes) ->
...
-spec try_publish(Host :: jlib:lserver(), topic_arn(), Content :: jiffy:json_value(),
...
%% @doc Publish notification to AWS SNS service. Content should be a valid JSON term
-spec publish(Host :: jlib:lserver(), topic_arn(), Content :: jiffy:json_value(),
...
-spec aws_handle(Host :: jlib:lserver()) -> aws_config().
aws_handle(Host) ->
...
-spec get_topic(Host :: jlib:lserver(), Packet :: exml:element()) -> topic() | undefined.
...
-spec make_topic_arn(Host :: jlib:lserver(), Topic :: topic()) -> topic_arn().
...
-spec opt(Host :: jlib:lserver(), Option :: atom()) -> Value :: term() | undefined.
opt(Host, Option) ->
    opt(Host, Option, undefined).

%% Getter for module options with default value
-spec opt(Host :: jlib:lserver(), Option :: atom(), Default :: term()) ->
    Value :: term().
...
-spec user_guid(Host :: jlib:lserver(), UserJID :: jlib:jid()) -> user_guid().
user_guid(Host, UserJID) ->
    PluginModule = opt(Host, plugin_module, mod_event_pusher_sns_defaults),
    PluginModule:user_guid(UserJID).

-spec message_attributes(Host :: jlib:lserver(), TopicARN :: topic_arn(),
                         UserJID :: jlib:jid(), IsOnline :: boolean()) ->
                                attributes().
...
-spec message_attributes(Host :: jlib:lserver(), TopicARN :: topic_arn(),
                         From :: jlib:jid(), To :: jlib:jid(), MessageType :: pm | muc,
                         Packet :: exml:element()) -> attributes().
...
-spec calc_backoff_time(Host :: jlib:lserver(), integer()) -> integer().
...
-spec pool_name(Host :: jlib:lserver()) -> atom().
pool_name(Host) ->
