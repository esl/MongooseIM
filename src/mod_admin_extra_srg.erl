...
-type group() :: binary().
-spec srg_create(group(), jlib:server(), jlib:user(),
...
-spec srg_delete(group(), jlib:server()) -> 'ok'.
srg_delete(Group, Host) ->
    {atomic, ok} = mod_shared_roster:delete_group(Host, Group),
    ok.


-spec srg_list(jlib:server()) -> [group()].
srg_list(Host) ->
    lists:sort(mod_shared_roster:list_groups(Host)).


-spec srg_get_info(group(), jlib:server()) -> [{string(), string()}].
...
-spec srg_get_members(group(), jlib:server()) -> [binary()].
...
-spec srg_user_add(jlib:user(), jlib:server(),
                   group(), GroupHost :: jlib:server()) -> 'ok'.
srg_user_add(User, Host, Group, GroupHost) ->
    {atomic, ok} = mod_shared_roster:add_user_to_group(GroupHost, {User, Host}, Group),
    ok.


-spec srg_user_del(jlib:user(), jlib:server(),
                   group(), GroupHost :: jlib:server()) -> 'ok'.
