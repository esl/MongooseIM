...
privacy_check_packet(Acc0, HostType, JID, PrivacyList, To, Dir) ->
    Acc1 = case Acc0 of
           {Acc, #xmlel{}} -> Acc;
            _ -> Acc0
        end,
    From = mongoose_acc:from_jid(Acc1),
    privacy_check_packet(Acc0, HostType, JID, PrivacyList, From, To, Dir).
...
-spec privacy_check_packet(Acc :: mongoose_acc:t() | {mongoose_acc:t(), exml:element()},
                           ...
                           To :: jid:jid(),
                           Dir :: 'in' | 'out') -> {mongoose_acc:t(), decision()}.
privacy_check_packet(Acc0, HostType, #jid{luser = User, server = Server} = JID,
    ...
    case mongoose_acc:get(privacy, Key, undefined, Acc) of
        undefined ->
            Acc1 = mongoose_hooks:privacy_check_packet(HostType, Acc, JID, PrivacyList,
