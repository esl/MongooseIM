...
-type status() :: binary().
-type u_s_r_p_st() :: { User    :: jlib:user(),
                        Server  :: jlib:server(),
                        Res     :: jlib:resource(),
                        Prio    :: integer(),
                        Status  :: status()}.
-type formatted_user_info() :: {USR :: string(),
                                ...
                                NodeS :: string(),
                                Uptime :: integer()}.
-type usr_nowpid_p_i() :: {jlib:simple_jid(),
...
-spec num_resources(jlib:user(), jlib:server()) -> non_neg_integer().
num_resources(User, Host) ->
    length(ejabberd_sm:get_user_resources(User, Host)).


-spec resource_num(jlib:user(), jlib:server(), integer()) -> binary() | string().
...
-spec kick_session(jlib:user(), jlib:server(), jlib:resource(),
...
-spec kick_this_session(jlib:user(), jlib:server(), jlib:resource(),
...
-spec status_num('all' | jlib:server(), status()) -> non_neg_integer().
...
-spec status_list('all' | jlib:server(), status()) -> [u_s_r_p_st()].
...
-spec get_status_list('all' | jlib:server(), status()) -> [u_s_r_p_st()].
...
-spec connected_users_info('all' | jlib:server()) -> [formatted_user_info()].
...
-spec set_presence(jlib:user(), jlib:server(), jlib:resource(),
...
-spec user_sessions_info(jlib:user(), jlib:server()) -> [formatted_user_info()].
