...
-spec get_session(Jid :: binary()) -> {ok, Host :: jlib:lserver()} | error.
get_session(Jid) ->
    do_get(Jid).
...
-spec get_domain(Domain :: binary()) -> {ok, Host :: jlib:lserver()} | error.
get_domain(Domain) ->
    do_get(Domain).
...
-spec get_endpoints(Host :: jlib:lserver()) -> {ok, [mod_global_distrib_utils:endpoint()]}.
get_endpoints(Host) ->
...
-spec nodes_key(Host :: jlib:lserver()) -> binary().
nodes_key(Host) ->
    <<Host/binary, "#{nodes}">>.
...
-spec endpoints_key(Host :: jlib:lserver(), Node :: binary()) -> binary().
...
-spec domains_key(Host :: jlib:lserver(), Node :: binary()) -> binary().
...
-spec key(Host :: jlib:lserver(), Node :: binary(), Type :: binary()) -> binary().
key(Host, Node, Type) ->
...
-spec do_get(Key :: binary()) -> {ok, Host :: jlib:lserver()} | error.
do_get(Key) ->
    case q([<<"GET">>, Key]) of
        {ok, undefined} -> error;
        {ok, Host} -> {ok, Host}
...
-spec get_hosts() -> [Host :: jlib:lserver()].
get_hosts() ->
...
-spec get_nodes(Host :: jlib:lserver()) -> [Node :: binary()].
get_nodes(Host) ->
