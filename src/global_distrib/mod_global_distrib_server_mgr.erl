...
-spec start_link(Server :: jlib:lserver(), ServerSup :: pid()) -> {ok, pid()} | {error, any()}.
...
-spec force_refresh(Server :: jlib:lserver()) -> ok.
force_refresh(Server) ->
    do_call(Server, force_refresh).

-spec close_disabled(Server :: jlib:lserver()) -> ok.
close_disabled(Server) ->
    do_call(Server, close_disabled).

-spec get_connection(Server :: jlib:lserver()) -> pid() | {error, any()}.
...
-spec get_enabled_endpoints(Server :: jlib:lserver()) -> [endpoint()].
get_enabled_endpoints(Server) ->
    do_call(Server, get_enabled_endpoints).

-spec get_disabled_endpoints(Server :: jlib:lserver()) -> [endpoint()].
...
-spec do_call(Server :: jlib:lserver(), Msg :: any()) -> any().
do_call(Server, Msg) ->
...
-spec get_endpoints(Server :: jlib:lserver()) -> {ok, [mod_global_distrib_utils:endpoint()]}.
