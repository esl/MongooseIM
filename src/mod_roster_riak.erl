...
-spec read_roster_version(jlib:luser(), jlib:lserver()) -> binary() | error.
...
-spec roster_entry_strip(jlib:simple_jid(), mod_roster:roster() | does_not_exist) ->
...
-spec get_t_roster(LUser :: jlib:luser(), LServer :: jlib:lserver()) ->
    riakc_map:crdt_map().
...
-spec set_t_roster(LUser :: jlib:luser(),
...
-spec del_t_roster(LUser :: jlib:luser(),
                   LServer :: jlib:lserver(),
                   LJID :: jlib:simple_jid()) -> any().
del_t_roster(LUser, LServer, LJID) ->
    RosterMap1 = get_t_roster(LUser, LServer),
