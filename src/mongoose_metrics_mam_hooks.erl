...
-spec mam_get_prefs(Result :: any(),
                    ...
                    _ArcJID :: jlib:jid()) -> any().
mam_get_prefs(Result, Host, _ArcID, _ArcJID) ->
    mongoose_metrics:update(Host, modMamPrefsGets, 1),
    Result.

-spec mam_set_prefs(Result :: any(), Host :: jlib:server(),
    _ArcID :: mod_mam:archive_id(), _ArcJID :: jlib:jid(),
    _DefaultMode :: any(), _AlwaysJIDs :: [jlib:literal_jid()],
    _NeverJIDs :: [jlib:literal_jid()]) -> any().
...
-spec mam_remove_archive(Acc :: map(),
                         Host :: jlib:server(),
                         _ArcID :: mod_mam:archive_id(),
                         _ArcJID :: jlib:jid()) -> metrics_notify_return().
...
-spec mam_archive_message(Result :: any(), Host :: jlib:server(),
    _MessId :: mod_mam:message_id(), _ArcID :: mod_mam:archive_id(),
    _LocJID :: jlib:jid(), _RemJID :: jlib:jid(),
    _SrcJID :: jlib:jid(), _Dir :: atom(), _Packet :: exml:element()) -> any().
...
%% #rh
-spec mam_drop_message(Acc :: map(), Host :: jlib:server()) -> metrics_notify_return().
mam_drop_message(Acc, Host) ->
    mongoose_metrics:update(Host, modMamDropped, 1),
    Acc.

%% #rh
-spec mam_drop_iq(Acc :: map(), Host :: jlib:server(), _To :: jlib:jid(),
...
-spec mam_muc_purge_single_message(Result :: any(), Host :: jlib:server(),
    _MessID :: mod_mam:message_id(), _ArcID :: mod_mam:archive_id(),
    _ArcJID :: jlib:jid(), _Now :: mod_mam:unix_timestamp()) -> any().
...
-spec mam_muc_purge_multiple_messages(Result :: any(),
    Host :: jlib:server(), _ArcID :: mod_mam:archive_id(),
    _ArcJID :: jlib:jid(), _Borders :: any(), _Start :: any(),
