...
-spec start(Host :: jlib:server(), MUCHost :: jlib:server()) -> ok.
start(_Host, _MUCHost) ->
    ok.

-spec stop(Host :: jlib:server(), MUCHost :: jlib:server()) -> ok.
stop(_Host, _MUCHost) ->
    ok.

%% ------------------------ General room management ------------------------

-spec create_room(RoomUS :: jlib:simple_bare_jid(), Config :: config(),
...
-spec destroy_room(RoomUS :: jlib:simple_bare_jid()) -> ok | {error, not_exists | not_empty}.
destroy_room(RoomUS) ->
    MainHost = main_host(RoomUS),
    {atomic, Res}
...
-spec room_exists(RoomUS :: jlib:simple_bare_jid()) -> boolean().
...
-spec get_user_rooms(UserUS :: jlib:simple_bare_jid(),
...
-spec get_user_rooms_count(UserUS :: jlib:simple_bare_jid(),
...
-spec remove_user(UserUS :: jlib:simple_bare_jid(), Version :: binary()) ->
...
-spec get_config(RoomUS :: jlib:simple_bare_jid()) ->
...
-spec get_config(RoomUS :: jlib:simple_bare_jid(), Key :: atom()) ->
...
-spec set_config(RoomUS :: jlib:simple_bare_jid(), Config :: config(), Version :: binary()) ->
...
-spec set_config(RoomUS :: jlib:simple_bare_jid(),
...
-spec get_blocking(UserUS :: jlib:simple_bare_jid(), MUCServer :: jlib:lserver()) ->
    [blocking_item()].
...
-spec get_blocking(UserUS :: jlib:simple_bare_jid(),
                   MUCServer :: jlib:lserver(),
                   WhatWhos :: [{blocking_who(), jlib:simple_bare_jid()}]) ->
    blocking_action().
...
-spec set_blocking(UserUS :: jlib:simple_bare_jid(),
                   MUCServer :: jlib:lserver(),
...
-spec get_aff_users(RoomUS :: jlib:simple_bare_jid()) ->
    {ok, aff_users(), Version :: binary()} | {error, not_exists}.
get_aff_users({RoomU, RoomS} = RoomUS) ->
    MainHost = main_host(RoomUS),
    case mongoose_rdbms:sql_query(MainHost, mod_muc_light_db_odbc_sql:select_affs(RoomU, RoomS)) of
        {selected, []} ->
            {error, not_exists};
...
-spec modify_aff_users(RoomUS :: jlib:simple_bare_jid(),
...
-spec get_info(RoomUS :: jlib:simple_bare_jid()) ->
...
-spec create_room_transaction(RoomUS :: jlib:simple_bare_jid(),
...
-spec destroy_room_transaction(RoomUS :: jlib:simple_bare_jid()) -> ok | {error, not_exists}.
...
-spec remove_user_transaction(UserUS :: jlib:simple_bare_jid(), Version :: binary()) ->
...
-spec set_config_transaction(RoomUS :: jlib:simple_bare_jid(),
...
-spec modify_aff_users_transaction(RoomUS :: jlib:simple_bare_jid(),
                                   AffUsersChanges :: aff_users(),
...
modify_aff_users_transaction(RoomUS, RoomID, AffUsersChanges, CheckFun, PrevVersion, Version) ->
    {selected, AffUsersDB}
    = mongoose_rdbms:sql_query_t(mod_muc_light_db_odbc_sql:select_affs(RoomID)),
    AffUsers = lists:sort(
...
-spec apply_aff_users_transaction(RoomID :: binary(),
                                  AffUsersChanges :: aff_users(),
                                  JoiningUsers :: [jlib:simple_bare_jid()]) -> ok.
apply_aff_users_transaction(RoomID, AffUsersChanged, JoiningUsers) ->
    lists:foreach(
...
-spec update_room_version_transaction(RoomUS :: jlib:simple_bare_jid(), Version :: binary()) ->
    {updated, integer()}.
...
-spec main_host(JIDOrServer :: jlib:simple_bare_jid() | binary()) -> jlib:lserver().
main_host({_, RoomS}) ->
    main_host(RoomS);
main_host(MUCServer) ->
