-module(mod_stream_management_kiss).
...
register_smid(_HostType, SMID, SID) ->
    kiss:insert(?TABLE, [{{sid, SID}, SMID}, {{smid, SMID}, SID}]).
...
unregister_smid(_HostType, SID) ->
    case ets:lookup(?TABLE, {sid, SID}) of
        ...
        [{_, SMID}] ->
            kiss:delete_many(?TABLE, [{sid, SID}, {smid, SMID}]),
            {ok, SMID}
    end.
