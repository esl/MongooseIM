...
-type error() :: {error, binary() | {binary(), binary()}}
               | {error, binary() | {binary(), binary()}, jlib:user()}.

-export_type([mechanism/0,
              password_type/0,
              error/0]).

-callback mech_new(Host :: jlib:server(),
...
-spec listmech(jlib:server()) -> [mechanism()].
listmech(Host) ->
...
%% given host
-spec filter_mechanisms(jlib:server(), [mechanism()],
