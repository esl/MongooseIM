...
-include("jlib.hrl").
-record(anonymous, {us  :: jlib:simple_bare_jid(),
                    sid :: ejabberd_sm:sid()
                   }).
...
-spec allow_anonymous(Host :: jlib:lserver()) -> boolean().
...
-spec is_sasl_anonymous_enabled(Host :: jlib:lserver()) -> boolean().
...
-spec is_login_anonymous_enabled(Host :: jlib:lserver()) -> boolean().
...
-spec anonymous_protocol(Host :: jlib:lserver()) ->
...
%% defaults to false
-spec allow_multiple_connections(Host :: jlib:lserver()) -> boolean().
...
-spec anonymous_user_exist(LUser :: jlib:luser(),
...
-spec purge_hook(boolean(), jlib:luser(), jlib:lserver()) -> 'ok'.
...
-spec session_cleanup(Acc :: map(), LUser :: jlib:luser(),
                      LServer :: jlib:lserver(),
...
-spec get_vh_registered_users(LServer :: jlib:lserver(), Opts :: list()) ->
...
-spec remove_user(LUser :: jlib:luser(),
                  LServer :: jlib:lserver()) -> {error, not_allowed}.
