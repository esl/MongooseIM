...
-spec start_pm(jlib:server(), _) -> 'ok'.
start_pm(Host, _Opts) ->
...
-spec archive_size(Size :: integer(), Host :: jlib:server(),
                   ArcId :: mod_mam:archive_id(), ArcJID :: jlib:jid()) -> integer().
...
-spec index_hint_sql(jlib:server()) -> string().
index_hint_sql(Host) ->
...
-spec archive_message(_Result, Host :: jlib:server(),
                      MessID :: mod_mam:message_id(), UserID :: mod_mam:archive_id(),
                      LocJID :: jlib:jid(), RemJID :: jlib:jid(),
                      SrcJID :: jlib:jid(), Dir :: atom(), Packet :: any()) -> ok.
...
-spec write_message(Host :: jlib:server(), Table :: string(),
...
-type msg() :: {binary(), jlib:literal_jid(), binary()}.
...
-spec prepare_filter(Host :: jlib:server(), UserID :: mod_mam:archive_id(),
                     UserJID :: jlib:jid(), Borders :: mod_mam:borders(),
                     Start :: mod_mam:unix_timestamp() | undefined,
                     End :: mod_mam:unix_timestamp() | undefined, WithJID :: jlib:jid(),
...
-spec db_jid_codec(jlib:server()) -> module().
db_jid_codec(Host) ->
    gen_mod:get_module_opt(Host, ?MODULE, db_jid_format, mam_jid_mini).

-spec db_message_codec(jlib:server()) -> module().
