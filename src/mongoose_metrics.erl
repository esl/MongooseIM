...
-spec init_predefined_host_metrics(jlib:lserver()) -> ok.
...
-spec create_generic_hook_metric(jlib:lserver(), atom()) -> ok | {ok, already_present}.
...
-spec update(Host :: jlib:lserver() | global, Name :: term() | list(),
             Change :: term()) -> any().
...
-spec ensure_metric(jlib:lserver() | global, atom() | list(), term()) -> ok | {error, term()}.
...
-spec increment_generic_hook_metric(jlib:lserver(), atom()) -> ok | {error, any()}.
...
-spec name_by_all_metrics_are_global(Host :: jlib:lserver() | global,
...
-spec do_create_generic_hook_metric(Host :: jlib:lserver() | global, Metric :: list()) ->
...
-spec do_increment_generic_hook_metric(Host :: jlib:lserver() | global, Name :: list()) ->
    ok | {error, any()}.
...
-spec create_metrics(jlib:server()) -> 'ok'.
create_metrics(Host) ->
...
-spec metrics_hooks('add' | 'delete', jlib:server()) -> 'ok'.
