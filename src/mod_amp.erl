...
-spec check_packet(exml:element()|mongoose_acc:t(), ejabberd:jid(), amp_event()) ->
...
-spec amp_check_packet(mongoose_acc:t(), ejabberd:jid(), amp_event()) -> mongoose_acc:t().
...
-spec process_amp_rules(exml:element(), ejabberd:jid(), amp_event(), amp_rules()) -> exml:element() | drop.
...
-spec determine_strategy(exml:element(), ejabberd:jid(), amp_event()) -> amp_strategy().
...
-spec fold_apply_rules(exml:element(), ejabberd:jid(), amp_strategy(), [amp_rule()]) ->
...
-spec resolve_condition(ejabberd:jid(), amp_strategy(), amp_rule()) -> amp_match_result().
...
-spec take_action_for_matched_rule(exml:element(), ejabberd:jid(), amp_rule()) -> exml:element() | drop.
...
-spec reply_to_sender(amp_rule(), ejabberd:jid(), ejabberd:jid(), exml:element()) -> mongoose_acc:t().
...
-spec send_error_and_drop(exml:element(), ejabberd:jid(), amp_error(), amp_rule()) -> drop.
send_error_and_drop(Packet, From, AmpError, MatchedRule) ->
    send_errors_and_drop(Packet, From, [{AmpError, MatchedRule}]).

-spec send_errors_and_drop(exml:element(), ejabberd:jid(), [{amp_error(), amp_rule()}]) -> drop.
...
-spec update_metric_and_drop(exml:element(), ejabberd:jid()) -> drop.
update_metric_and_drop(Packet, From) ->
...
-spec host(ejabberd:jid()) -> binary().
host(#jid{lserver=Host}) -> Host.

server_jid(#jid{lserver = Host}) ->
    jid:from_binary(Host).

-spec message_target(exml:element()) -> ejabberd:jid() | undefined.
message_target(El) ->
