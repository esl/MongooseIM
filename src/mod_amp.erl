...
run_initial_check(Acc) ->
    ...
    case Result of
        ...
        NewRules ->
            Acc1 = mongoose_acc:append(amp, state, {Event, NewRules}, Acc),
            mongoose_acc:update_stanza(#{element => strip_amp_el_from_request(Packet)}, Acc1)
    end.
...
process_event(Acc, [{_LastEvent, Rules} | _], Event) ->
    ...
    mongoose_acc:append(amp, state, {Event, NewRules}, Acc).
...
-spec validate_and_process_rules(exml:element(), jid:jid(), amp_event(), amp_rules()) ->
          exml:element() | drop.
