...
-callback get_job(mongooseim:host_type(), JobId :: integer()) ->
    {ok, broadcast_job()} | {error, not_found | term()}.

-callback get_running_jobs(mongooseim:host_type()) ->
    {ok, [broadcast_job()]} | {error, term()}.

-callback get_worker_state(mongooseim:host_type(), JobId :: integer()) ->
    {ok, broadcast_worker_state()} | {error, not_found | term()}.

-callback set_job_started(mongooseim:host_type(), JobId :: integer()) ->
    ok | {error, term()}.

-callback update_worker_state(mongooseim:host_type(), JobId :: integer(),
                              WorkerState :: broadcast_worker_state()) ->
    ok | {error, term()}.

-callback set_job_finished(mongooseim:host_type(), JobId :: integer()) ->
    ok | {error, term()}.

-callback set_job_aborted(mongooseim:host_type(), JobId :: integer(), Reason :: binary()) ->
    ok | {error, term()}.
