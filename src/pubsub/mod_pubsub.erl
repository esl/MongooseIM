...
handle_msg({send_last_items_from_owner, HostType, Host, NodeOwner, RecipientJID}) ->
...
out_subscription(Acc, #{to := ToJID, from := FromJID, type := subscribed}, _) ->
    ...
    notify_worker(LServer, LUser, {send_last_items_from_owner, HostType, Host, FromJID, ToJID}),
    {ok, Acc};
...
broadcast_stanza({LUser, LServer, LResource}, ReplyToJID, Node, Nidx, Type, NodeOptions,
    ...
    case ejabberd_sm:get_session_pid(jid:make(LUser, LServer, SenderResource)) of
        C2SPid when is_pid(C2SPid) ->
            ...
            HostType = mongoose_acc:host_type(Acc),
            [route_broadcasted(HostType, FromJid, ToJid, Packet) ||
                Recipient <- mongoose_acc:get(?MODULE, presence_recipients, [], Acc),
                ToJid <- full_jids_with_feature(HostType, Recipient, Feature)],
            ok;
        _ ->
