...
process_map_spec(KVs) ->
    {[[{vcard_field, VF}],
      [{ldap_pattern, LP}],
      [{ldap_field, LF}]], []} = proplists:split(KVs, [vcard_field, ldap_pattern, ldap_field]),
    {VF, LP, [LF]}.

process_search_spec(KVs) ->
    {[[{search_field, SF}],
      [{ldap_field, LF}]], []} = proplists:split(KVs, [search_field, ldap_field]),
    {SF, LF}.

process_search_reported_spec(KVs) ->
    {[[{search_field, SF}],
      [{vcard_field, VF}]], []} = proplists:split(KVs, [search_field, vcard_field]),
    {SF, VF}.
