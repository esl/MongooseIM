...
-spec srv_name(jlib:server() | string()) -> atom().
srv_name(Host) ->
...
-spec tbl_name(jlib:server() | string()) -> atom().
tbl_name(Host) ->
...
-spec does_user_exist(LUser :: jlib:luser(),
                     LServer :: jlib:lserver() | string()) -> boolean().
...
-spec remove_user(Acc :: mongoose_acc:t(),
                  LUser :: jlib:luser(),
                  LServer :: jlib:lserver() | string()) -> mongoose_acc:t().
...
%%--------------------------------------------------------------------
-spec init([jlib:server() | string(), ...]) -> {'ok', #state{}}.
init([Host]) ->
    Tab = tbl_name(Host),
...
-spec does_stored_user_exist(jlib:luser(), jlib:lserver()) -> boolean().
...
-spec is_anonymous_user(jlib:luser(), jlib:lserver()) -> boolean().
...
-spec does_cached_user_exist(jlib:luser(), jlib:lserver() | string()) -> boolean().
...
-spec put_user_into_cache(jlib:luser(), jlib:lserver()) -> 'ok'.
put_user_into_cache(LUser, LServer) ->
    Key = key(LUser, LServer),
    Tab = tbl_name(LServer),
    ets:insert(Tab, {Key}),
...
-spec delete_user(jlib:luser(), jlib:lserver() | string()) -> 'ok'.
delete_user(LUser, LServer) ->
    Key = key(LUser, LServer),
    Tab = tbl_name(LServer),
    ets:delete(Tab, Key),
...
-spec key(jlib:luser(), jlib:lserver()
         ) -> {jlib:luser(), jlib:lserver()}.
key(LUser, LServer) ->
    {LUser, LServer}.
