...
-spec check_hosts([jlib:server()], [jlib:server()]) -> {[jlib:server()],
                                                                [jlib:server()]}.
...
-spec add_virtual_host(Host :: jlib:server()) -> any().
...
-spec remove_virtual_host(jlib:server()) -> any().
...
%% match all hosts
-spec get_host_local_config() -> [{local_config, {term(), jlib:server()}, term()}].
get_host_local_config() ->
