...
-spec get_behaviour(Default :: mod_mam:archive_behaviour(),
        Host :: jlib:server(), ArchiveID :: mod_mam:archive_id(),
...
%% #rh
-spec remove_archive(map(), jlib:server(), mod_mam:archive_id(),
                     jlib:jid()) -> map().
...
-spec query_behaviour(jlib:server(), SUserID :: string(),
...
-spec esc_jid(jlib:simple_jid() | jlib:jid()) -> binary().
esc_jid(JID) ->
...
-spec sql_transaction_map(jlib:server(), [iolist(), ...]) -> any().
...
-spec decode_prefs_rows([{binary() | jlib:jid(), binary()}],
        DefaultMode :: mod_mam:archive_behaviour(),
        AlwaysJIDs :: [jlib:literal_jid()],
        NeverJIDs :: [jlib:literal_jid()]) ->
    {mod_mam:archive_behaviour(), [jlib:literal_jid()], [jlib:literal_jid()]}.
