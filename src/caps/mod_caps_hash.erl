...
encode_group(Version, Group, PreparedElements) ->
    case deduplicate_group(Group, PreparedElements) of
        Dedup when length(Dedup) =:= length(PreparedElements) ->
            encode_group(Version, [encode(Version, Group, El) || El <- Dedup]);
        Dedup ->
            ...
            error({duplicate_elements, PreparedElements -- Dedup})
    end.
...
%% XEP-0390 4-6 requires sorting by octet strings
