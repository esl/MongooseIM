...
rsm_decode(#xmlel{} = SubEl) ->
    case exml_query:subelement_with_name_and_ns(SubEl, <<"set">>, ?NS_RSM) of
        undefined ->
            none;
...
rsm_parse_element(#xmlel{name = <<"max">>, attrs = []} = Elem, RsmIn) ->
    ...
    Count = mongoose_lib:maybe_binary_to_positive_integer(CountStr),
    RsmIn#rsm_in{max = Count};
rsm_parse_element(#xmlel{name = <<"index">>, attrs = []} = Elem, RsmIn) ->
    IndexStr = exml_query:cdata(Elem),
    Index = mongoose_lib:maybe_binary_to_positive_integer(IndexStr),
