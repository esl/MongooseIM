...
stop(HostType) ->
    gen_iq_handler:remove_iq_handler_for_domain(HostType, ?NS_PRIVACY, ejabberd_sm),
...
c2s_hooks(HostType) ->
    [
     ...
     {user_received_message, HostType, fun ?MODULE:user_received_message/3, #{}, 10},
     {user_received_presence, HostType, fun ?MODULE:user_received_presence/3, #{}, 10},
     {user_received_iq, HostType, fun ?MODULE:user_received_iq/3, #{}, 10},
...
-spec user_send_message(mongoose_acc:t(), mongoose_c2s:hook_params(), gen_hook:extra()) ->
    mongoose_c2s_hooks:hook_result().
user_send_message(Acc, #{c2s_data := StateData}, _Extra) ->
    do_privacy_check_send(Acc, StateData).

-spec user_send_presence(mongoose_acc:t(), mongoose_c2s:hook_params(), gen_hook:extra()) ->
    mongoose_c2s_hooks:hook_result().
...
-spec user_received_message(mongoose_acc:t(), mongoose_c2s:hook_params(), gen_hook:extra()) ->
    mongoose_c2s_hooks:hook_result().
user_received_message(Acc, #{c2s_data := StateData}, _Extra) ->
...
-spec user_received_iq(mongoose_acc:t(), mongoose_c2s:hook_params(), gen_hook:extra()) ->
    mongoose_c2s_hooks:hook_result().
user_received_iq(Acc, #{c2s_data := StateData}, _Extra) ->
    case mongoose_acc:stanza_type(Acc) of
...
handle_new_privacy_list(Acc, StateData, HostType, PrivList, PrivListName) ->
    ...
    case mongoose_hooks:privacy_updated_list(HostType, OldPrivList, PrivList) of
        ...
        NewPrivList ->
            ...
            ToAcc = [{socket_send, PrivPushEl}, {handlers, {?MODULE, NewPrivList}}],
...
process_privacy_iq(Acc, HostType, set, ToJid, StateData) ->
    ...
    case mongoose_acc:get(hook, result, undefined, Acc2) of
        {result, _, NewPrivList} ->
            ...
            mongoose_c2s_acc:to_acc(Acc2, handlers, {?MODULE, NewPrivList});
        _ ->
            Acc2
    end.
...
maybe_update_presence(Acc, StateData, OldList, NewList) ->
    Jid = mongoose_c2s:get_jid(StateData),
    Presences = mongoose_c2s:get_handler(StateData, mod_presence),
...
get_handler(StateData) ->
    case mongoose_c2s:get_handler(StateData, ?MODULE) of
