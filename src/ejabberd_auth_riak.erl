...
-spec start(jlib:lserver()) -> ok.
start(_Host) ->
    ok.

-spec stop(jlib:lserver()) -> ok.
stop(_Host) ->
    ok.

-spec store_type(jlib:lserver()) -> plain | scram.
store_type(Host) ->
...
-spec set_password(jlib:luser(), jlib:lserver(), binary())
...
-spec check_password(jlib:luser(), jlib:lserver(), binary()) -> boolean().
...
-spec check_password(jlib:luser(),
                     jlib:lserver(),
                     binary(),
                     binary(),
                     fun()) -> boolean().
...
-spec try_register(User :: jlib:luser(),
                   Server :: jlib:lserver(),
                   Password :: binary()
                  ) -> ok | {error, term()}.
...
-spec get_vh_registered_users(jlib:lserver()) ->
...
-spec get_vh_registered_users(jlib:lserver(), list()) ->
    [jlib:simple_bare_jid()].
get_vh_registered_users(LServer, _Opts) ->
    get_vh_registered_users(LServer).

-spec get_vh_registered_users_number(jlib:lserver()) -> non_neg_integer().
get_vh_registered_users_number(LServer) ->
    length(get_vh_registered_users(LServer)).

-spec get_vh_registered_users_number(jlib:lserver(), list()) -> non_neg_integer().
get_vh_registered_users_number(LServer, _Opts) ->
    get_vh_registered_users_number(LServer).

-spec get_password(jlib:luser(), jlib:lserver()) -> ejabberd_auth:passterm() | false.
...
-spec does_user_exist(jlib:luser(), jlib:lserver()) -> boolean().
...
-spec remove_user(jlib:luser(), jlib:lserver()) ->
...
-spec bucket_type(jlib:lserver()) -> {binary(), jlib:lserver()}.
bucket_type(LServer) ->
    {<<"users">>, LServer}.
