...
-spec user_send_packet(mongoose_acc:t(), jid(), jid(), exml:element()) -> mongoose_acc:t().
...
-spec user_receive_packet(mongoose_acc:t(), jid(), jid(), jid(), exml:element()) -> mongoose_acc:t().
...
-spec feature_response(mongoose_acc:t(), jlib:iq(), ejabberd:server(), jid(), caps(), [binary()]) ->
    mongoose_acc:t().
