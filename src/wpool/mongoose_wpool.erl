...
%% Mostly for tests
-export([expand_pools/3, instrumentation/3]).

-ignore_xref([call/2, cast/2, cast/3, expand_pools/3, instrumentation/3, get_worker/2,
              send_request/2, send_request/3, send_request/4, send_request/5,
...
-spec default_instrumentation(pool_type(), host_type_or_global(), tag()) -> [mongoose_instrument:spec()].
default_instrumentation(PoolType, global, Tag) ->
    [{wpool_global_queue_lengths, #{pool_type => PoolType, pool_tag => Tag},
      #{probe => #{module => ?MODULE},
        metrics => #{total => gauge}}}];
default_instrumentation(PoolType, HostType, Tag) ->
    [{wpool_queue_lengths, #{host_type => HostType, pool_type => PoolType, pool_tag => Tag},
      #{probe => #{module => ?MODULE},
        metrics => #{total => gauge}}}].
