...
mod_event_pusher_push_rules(_Config) ->
    ...
    RawRule = #{~"conditions" => #{~"event" => ~"chat"},
                ~"action" => ~"push",
                ~"content" => ~"message"},
    Rule = #{conditions => #{event => chat}, action => push, content => message},
    ?cfgh(P ++ [rules], [Rule], T([RawRule])),
    ?cfgh(P ++ [rules], [Rule#{conditions := #{has_body => true}}],
          T([RawRule#{~"conditions" := #{~"has_body" => true}}])),
    ...
    ?errh(T([RawRule#{~"conditions" := #{~"has_body" => ~"yes"}}])),
