...
mod_stream_management(_Config) ->
    ...
    ?eqf(M([{buffer_max, 100}]),
        T(#{<<"buffer_max">> => 100})),
    ?eqf(M([{ack_freq, 1}]),
        T(#{<<"ack_freq">> => 1})),
    ?eqf(M([{resume_timeout, 600}]),
        T(#{<<"resume_timeout">> => 600})),
    ?eqf(M([{buffer_max, 100}]),
        T(#{<<"buffer_max">> => 100})),
    ?eqf(M([{buffer_max, 100}]),
        T(#{<<"buffer_max">> => 100})),
    StaleH = #{<<"stale_h">> => #{<<"enabled">> => true,
                                  <<"repeat_after">> => 1800,
                                  <<"geriatric">> => 3600}},
    MStaleH = [{stale_h, [{enabled, true},
                          {stale_h_geriatric, 3600},
                          {stale_h_repeat_after, 1800}]}],
    ?eqf(modopts(mod_stream_management, MStaleH), T(StaleH)),
