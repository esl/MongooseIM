...
mod_event_pusher_push_rules(_Config) ->
    P = [modules, mod_event_pusher, push],
    T0 = fun(Opts) -> #{~"modules" => #{~"mod_event_pusher" => #{~"push" => Opts}}} end,
    T = fun(RawRules) -> T0(#{~"rules" => RawRules}) end,
    RawRule = #{~"conditions" => [#{~"event" => ~"msg"}],
                ~"action" => ~"push",
                ~"content" => ~"message"},
    Rule = #{conditions => [#{event => msg}], action => push, content => message},
    ?cfgh(P ++ [rules], [Rule], T([RawRule])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{event => msg}, #{event => unack_msg}]}],
          T([RawRule#{~"conditions" := [#{~"event" => ~"msg"},
                                          #{~"event" => ~"unack_msg"}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{type => groupchat}]}],
          T([RawRule#{~"conditions" := [#{~"type" => ~"groupchat"}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{body => non_empty}]}],
          T([RawRule#{~"conditions" := [#{~"body" => ~"non_empty"}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{hint => store}]}],
          T([RawRule#{~"conditions" := [#{~"hint" => ~"store"}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{jingle => true}]}],
          T([RawRule#{~"conditions" := [#{~"jingle" => true}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{user_status => offline}]}],
          T([RawRule#{~"conditions" := [#{~"user_status" => ~"offline"}]}])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{client_state => inactive}]}],
          T([RawRule#{~"conditions" := [#{~"client_state" => ~"inactive"}]}])),
    ?cfgh(P ++ [rules], [], T([])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{}]}],
          T([maps:remove(~"conditions", RawRule)])),
    SkipRule = maps:remove(content, Rule#{action := skip}),
    SkipRawRule = maps:remove(~"content", RawRule#{~"action" := ~"skip"}),
    ?cfgh(P ++ [rules], [SkipRule], T([SkipRawRule])),
    ?errh(T(#{})),
    ?errh(T([maps:remove(~"action", RawRule)])),
    ?errh([#{reason := missing_push_rule_content}],
          T([maps:remove(~"content", RawRule)])),
    ?errh([#{reason := unsupported_push_rule_content}],
          T([RawRule#{~"action" := ~"skip"}])),
    ?errh(T([RawRule, RawRule])),
    ?errh(T([RawRule#{~"conditions" := []}])),
    ?errh(T([RawRule#{~"conditions" := [#{}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"event" => ~"msg"},
                                          #{~"event" => ~"msg"}]}])),
    ?errh(T([RawRule#{~"action" := ~"notify"}])),
    ?errh(T([RawRule#{~"content" := ~"summary"}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"event" => ~"presence"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"type" => ~"error"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"type" => [~"chat"]}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"body" => ~"present"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"hint" => ~"archive"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"jingle" => ~"yes"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"user_status" => ~"away"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"user_status" => [~"offline"]}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"client_state" => ~"away"}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"client_state" => [~"inactive"]}]}])),
    ?errh([#{reason := invalid_push_options}],
          T0(#{~"plugin_module" => ~"mod_event_pusher_push_plugin_defaults",
               ~"rules" => [RawRule]})).
