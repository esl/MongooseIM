...
listen_c2s_just_tls(_Config) ->
    ...
    ?cfg(P ++ [session_tickets], stateless, T(M#{<<"session_tickets">> => <<"stateless">>})),
    ?cfg(P ++ [early_data], true, T(M#{<<"early_data">> => true})),
    ?cfg(P ++ [early_data], false, T(M#{<<"early_data">> => false})),
    ...
    ?err(T(M#{<<"session_tickets">> => <<"something">>})),
    ?err(T(M#{<<"early_data">> => <<"enabled">>})).
