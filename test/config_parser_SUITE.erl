...
mod_event_pusher_push_rules(_Config) ->
    ...
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{event => msg}, #{event => unack_msg}]}],
          T([RawRule#{~"conditions" := [#{~"event" => ~"msg"}, #{~"event" => ~"unack_msg"}]}])),
    ...
    ?cfgh(P ++ [rules], [], T([])),
    ?cfgh(P ++ [rules], [Rule#{conditions := [#{}]}], T([maps:remove(~"conditions", RawRule)])),
    ...
    ?errh([#{reason := missing_push_rule_content}], T([maps:remove(~"content", RawRule)])),
    ?errh([#{reason := unsupported_push_rule_content}], T([RawRule#{~"action" := ~"skip"}])),
    ?errh(T([RawRule, RawRule])),
    ?errh(T([RawRule#{~"conditions" := []}])),
    ?errh(T([RawRule#{~"conditions" := [#{}]}])),
    ?errh(T([RawRule#{~"conditions" := [#{~"event" => ~"msg"}, #{~"event" => ~"msg"}]}])),
