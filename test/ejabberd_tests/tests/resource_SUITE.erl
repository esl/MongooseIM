...
groups() ->
    [{conflict, [sequence], [same_resource_conflict,
                             same_on_another_node]} ].
...
init_per_group(_GroupName, Config) ->
    escalus:create_users(Config, escalus:get_users([alice, alice_b])).

end_per_group(_GroupName, Config) ->
    escalus:delete_users(Config, escalus:get_users([alice, alice_b])).
...
same_on_another_node(Config) ->
    EUs = proplists:get_value(escalus_users, Config),
    UserSpec = proplists:get_value(alice, EUs),
    UserSpecB = proplists:get_value(alice_b, EUs),
    Resource = <<"res1">>,
    {ok, Client} = escalus_client:start(Config, UserSpec, Resource),
    {ok, _Client2} = escalus_client:start(Config, UserSpecB, Resource),
    E = escalus:wait_for_stanza(Client),
    escalus:assert(is_stream_error, [<<"conflict">>, <<"Replaced by new connection">>], E).
