...
prop_aff_change_success() ->
    ?FORALL({AffUsers, Changes, Joining, Leaving, WithOwner}, change_aff_params(),
            begin
                case mod_muc_light_utils:change_aff_users(<<"localhost">>, AffUsers, Changes) of
                    {ok, NewAffUsers0, AffUsersChanged, Joining0, Leaving0} ->
                        ...
                        {ok, NewAffUsers1, _, _, _} = mod_muc_light_utils:change_aff_users(<<"localhost">>, AffUsers, AffUsersChanged),
...
prop_aff_change_bad_request() ->
    ?FORALL({AffUsers, Changes}, bad_change_aff(),
            begin
                {error, {bad_request, _}} =
                    mod_muc_light_utils:change_aff_users(<<"localhost">>, AffUsers, Changes),
                true
            end).
