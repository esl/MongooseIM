    ...
    Config2 =
        lists:foldl(
          fun({NodeName, LocalHost, ReceiverPort}, Config1) ->
                  Opts = [{local_host, LocalHost}, {global_host, "localhost"}, {endpoints, [{{127, 0, 0, 1}, ReceiverPort}]}, {tls_opts, [{certfile, ?config(certfile, Config1)}, {cafile, ?config(cafile, Config1)}]}],
