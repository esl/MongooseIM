...
disco_info_node_story(Alice, Bob, Kate) ->
    ...
    pubsub_tools:publish(Alice, ~"item1", pep_node(Alice, NodeNS), []),
...
disco_items_open_story(Alice, Bob, Kate) ->
    ...
    pubsub_tools:publish(Alice, ~"item1", PepNode, []),
    set_up_presence_subscription(Bob, Alice, 0),
...
auto_create_and_publish_self_notify_story(Config, Bob) ->
    ...
    pubsub_tools:publish(Bob, ~"item1", PepNode, [{expected_notification, {PepNode, ~"item1"}}]),
    ...
    {Response, Msg} = pubsub_tools:publish(Bob, undefined, PepNode, [{expected_notification, PepNode}]),
    GeneratedItemId = pubsub_tools:published_item_id(Response, PepNode),
    pubsub_tools:check_item_notification(Msg, GeneratedItemId, PepNode, []),
    pubsub_tools:get_item(Bob, PepNode, GeneratedItemId, [{expected_result, [GeneratedItemId]}]).

auto_create_open_and_publish_explicit_sub_story(_Config, Alice, Bob) ->
    NodeNS = random_node_ns(),
    PepNode = pep_node(Alice, NodeNS),
    PublishOptions = [{~"pubsub#access_model", ~"open"}],
    pubsub_tools:publish_with_options(Alice, ~"item0", PepNode, [], PublishOptions),
    pubsub_tools:publish(Alice, ~"item1", PepNode, []),

    %% XEP-0163 4.3.4 Bob can subscribe and receive the last item
    pubsub_tools:subscribe(Bob, PepNode, [{expected_notification, {PepNode, ~"item1"}}]),
    ...
    %% Malformed retrieve-items request
    InvalidItemsOpts = [{expected_error_type, {~"modify", ~"bad-request"}}],
    pubsub_tools:get_items(Bob, PepNode, [~"item1", undefined], InvalidItemsOpts).
...
create_open_and_publish_both_subs_story(Config, Alice, Bob) ->
    ...
    pubsub_tools:subscribe(Bob, PepNode, []),
    pubsub_tools:subscribe(Bob, PepNode, [{jid_type, bare}]),
...
create_and_delete_node_story(Alice, Bob) ->
    ...
    pubsub_tools:create_node(Alice, PepNodeNoSubs, []),
    pubsub_tools:delete_node(Alice, PepNodeNoSubs, []).

create_node_with_empty_config_story(Alice) ->
    PepNode = pep_node(Alice),
    pubsub_tools:create_node(Alice, PepNode, [{config, []}]),
...
create_and_configure_node_story(Alice) ->
    ...
    pubsub_tools:set_configuration(Alice, PepNode, DefaultConfig, []),
    pubsub_tools:get_configuration(Alice, PepNode, [{expected_result, DefaultConfig}]),
    ...
    pubsub_tools:set_configuration(Alice, PepNode, UpdatedConfig, MalformedConfigOpts),

    %% XEP-0060 8.2.5.1 Success
    pubsub_tools:set_configuration(Alice, PepNode, UpdatedConfig, []),
    pubsub_tools:get_configuration(Alice, PepNode, [{expected_result, UpdatedConfig}]).
...
send_last_item_on_implicit_sub_story(Config, Alice, Bob) ->
    ...
    NodeNS = ?config(node_ns, Config),
    PepNode = pep_node(Alice, NodeNS),
    pubsub_tools:publish(Alice, ~"item1", PepNode, []), % previous item - not delivered
    pubsub_tools:publish(Alice, ~"item2", PepNode, []), % expected last item
    [Message] = set_up_presence_subscription(Bob, Alice, 1),
    pubsub_tools:check_item_notification(Message, ~"item2", PepNode, []),
    assert_delayed_notification(Message),

    %% Bob can receive further notifications
    pubsub_tools:publish(Alice, ~"item3", PepNode, []),
    pubsub_tools:receive_item_notification(Bob, ~"item3", PepNode, []).
...
request_without_nodeid_fails_story(Alice) ->
    ...
    pubsub_tools:create_node(Alice, MissingNode, CreateOpts),

    %% XEP-0060 8.5.3.2 NodeID Required for 'get'
    GetOpts = [{expected_error_type, {~"modify", ~"bad-request", ~"nodeid-required"}}],
    pubsub_tools:get_configuration(Alice, MissingNode, GetOpts),

    %% XEP-0060 8.5.3.2 NodeID Required for 'set'
    SetOpts = [{expected_error_type, {~"modify", ~"bad-request", ~"nodeid-required"}}],
    pubsub_tools:set_configuration(Alice, MissingNode, [{~"pubsub#access_model", ~"open"}], SetOpts).

bad_request_fails_story(Alice) ->
    Opts = [{expected_error_type, {~"modify", ~"bad-request"}}],
    pubsub_tools:send_generic_request(Alice, ~"set", ?NS_PUBSUB, ~"unknown-action", Opts),
    pubsub_tools:send_generic_request(Alice, ~"get", ?NS_PUBSUB, ~"unknown-action", Opts),
    pubsub_tools:send_generic_request(Alice, ~"set", ?NS_PUBSUB_OWNER, ~"unknown-action", Opts),
    pubsub_tools:send_generic_request(Alice, ~"get", ?NS_PUBSUB_OWNER, ~"unknown-action", Opts),
    pubsub_tools:send_generic_request(Alice, ~"set", ?NS_PUBSUB, {raw, ~"pubstub"}, Opts).
...
create_node_at_foreign_jid_fails_story(Alice, Bob) ->
    ...
    pubsub_tools:create_node(Bob, pep_node(Alice), Opts),

    %% XEP-0060 7.1.3.1 Insufficient Privileges (auto-create attempt)
    pubsub_tools:publish(Bob, ~"item2", pep_node(Alice), Opts).
...
create_node_invalid_config_fails_story(Alice) ->
    ...
    pubsub_tools:create_node(Alice, pep_node(Alice), InvalidOptionOpts),

    InvalidModelOpts = [{config, [{~"pubsub#access_model", ~"not-a-real-model"}]},
                        {expected_error_type, {~"modify", ~"bad-request"}}],
    pubsub_tools:create_node(Alice, pep_node(Alice), InvalidModelOpts),
    ...
    pubsub_tools:create_node(Alice, pep_node(Alice), ExtraChildOpts),
    ...
    pubsub_tools:create_node(Alice, pep_node(Alice), UnsupportedModelOpts).
...
manage_foreign_node_fails_story(Alice, Bob) ->
    ...
    pubsub_tools:delete_node(Bob, pep_node(Alice), Opts).
...
publish_with_invalid_items_fails_story(Alice) ->
    ...
    pubsub_tools:publish(Alice, ~"item1", PepNode, ItemRequiredOpts),

    %% XEP-0060 7.1.3.6 Payload Required
    EmptyItem = pubsub_tools:item_el(~"item2", []),
    PayloadRequiredOpts = [{expected_error_type, {~"modify", ~"bad-request", ~"payload-required"}}],
    pubsub_tools:publish_raw(Alice, PepNode, [EmptyItem], PayloadRequiredOpts),

    %% XEP-0060 7.1.3.6 Invalid Payload
    Content = pubsub_tools:item_content(),
    InvalidItem = pubsub_tools:item_el(~"item3", [Content, Content]),
    InvalidPayloadOpts = [{expected_error_type, {~"modify", ~"bad-request", ~"invalid-payload"}}],
    pubsub_tools:publish_raw(Alice, PepNode, [InvalidItem], InvalidPayloadOpts),

    %% XEP-0060 7.1.3.5 Too Many Items
    ItemEl = pubsub_tools:item_el(~"item4", [Content]),
    BadRequestOpts = [{expected_error_type, {~"modify", ~"bad-request"}}],
    pubsub_tools:publish_raw(Alice, PepNode, [ItemEl, ItemEl], BadRequestOpts),

    %% Malformed publish request: direct child is not <item/>.
    pubsub_tools:publish_raw(Alice, PepNode, [Content], BadRequestOpts).
...
publish_options_invalid_config_fails_story(Alice) ->
    ...
    pubsub_tools:publish_with_options(Alice, ~"item1", pep_node(Alice),
    ...
    pubsub_tools:publish_with_options(Alice, ~"item1", PepNode, PreconditionOpts, Fields),

    %% Malformed publish-options request with unexpected extra child
    MalformedOpts = [{expected_error_type, {~"modify", ~"bad-request"}},
                     {modify_request, fun pubsub_tools:add_unexpected_pubsub_child/1}],
    pubsub_tools:publish_with_options(Alice, ~"item1", pep_node(Alice), MalformedOpts, Fields),
