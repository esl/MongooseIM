...
list_contacts(Config) ->
    escalus:fresh_story(
        ...
        fun(Alice, Bob) ->
            ...
            % list bob's contacts
            {?OK, R} = gett(admin, lists:flatten(["/contacts?caller=", binary_to_list(BobJID)])),
            [R1] = decode_maplist(R),
...
befriend_and_alienate(Config) ->
    escalus:fresh_story(
        ...
        fun(Alice, Bob) ->
            ...
            AlicePath = lists:flatten(["/contacts?caller=", AliceS]),
            BobPath = lists:flatten(["/contacts?caller=", BobS]),
            % rosters are empty
            ...
            {?NOCONTENT, _} = putt(admin,
                                   lists:flatten(["/contacts/", BobS, "?caller=", AliceS]),
                                   #{action => <<"subscribe">>}),
            check_roster(AlicePath, BobJID, none, out),
            {?NOCONTENT, _} = putt(admin,
                                   lists:flatten(["/contacts/", AliceS, "?caller=", BobS]),
                                   #{action => <<"subscribed">>}),
            ...
            % now remove
            {?NOCONTENT, _} = delete(admin, lists:flatten(["/contacts/", BobS, "?caller=", AliceS])),
            check_roster_empty(AlicePath),
            check_roster(BobPath, AliceJID, none, none),
            {?NOCONTENT, _} = delete(admin, lists:flatten(["/contacts/", AliceS, "?caller=", BobS])),
            check_roster_empty(BobPath),
...
invalid_roster_operations(Config) ->
    escalus:fresh_story(
        ...
        fun(Alice, Bob) ->
            ...
            AlicePath = lists:flatten(["/contacts?caller=", AliceS]),
            % adds them to rosters
            {?BAD_REQUEST, <<"Invalid jid: \"@invalidjid\"">>} = post(admin, AlicePath, #{jid => <<"@invalidjid">>}),
            % it is idempotent
            {?NOCONTENT, _} = post(admin, AlicePath, #{jid => BobJID}),
            {?NOCONTENT, _} = post(admin, AlicePath, #{jid => BobJID}),
            PutPathA = lists:flatten(["/contacts/@invalidjid?caller=", AliceS]),
            {?BAD_REQUEST, <<"Invalid jid: \"@invalidjid\"">>} = putt(admin, PutPathA, #{action => <<"subscribe">>}),
            PutPathC = lists:flatten(["/contacts/", BobS, "?caller=", AliceS]),
            ...
            {?BAD_REQUEST, <<"Invalid jid: \"@invalid\"">>} = putt(admin, MangePathA, #{action => <<"connect">>}),
            ...
            {?BAD_REQUEST, <<"Invalid jid: \"@bzzz\"">>} = putt(admin, MangePathB, #{action => <<"connect">>}),
            ok
        end
    ).
