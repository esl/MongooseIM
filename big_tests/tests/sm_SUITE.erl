...
required_modules(group, parallel_large_buffer) ->
    SMConfig = #{buffer_max => 1000, ack_freq => never},
...
resend_unacked_from_stopped_sessions(Config) ->
    ...
    [sm_helper:wait_for_c2s_unacked_count(Pid, length(Texts) + length(Users)) || Pid <- C2SPids],
    ...
    lists:foldl(fun(User, {PresCount, MsgCount}) ->
                        escalus:wait_for_stanzas(User, PresCount + MsgCount),
                        timer:sleep(100),
                        escalus_assert:has_no_stanzas(User),
                        escalus_connection:stop(User),
                        {PresCount + 1, MsgCount * 2}
                end, {length(Users) - 1, length(Texts)}, Users),
    escalus_connection:stop(Bob).

%% Flaky because of presence vs resend race condition
resend_unacked_from_terminated_sessions(Config) ->
    ...
    C2SPids = lists:map(fun mongoose_helper:get_session_pid/1, Users),
    lists:foreach(fun wait_until_resume_session/1, C2SPids),

    %% Bob sends messages to User's bare jid
    %UserJid = escalus_users:get_jid(Config, hd(UserSpecs)),
    sm_helper:send_messages(Bob, hd(Users), Texts),
    [sm_helper:wait_for_c2s_unacked_count(Pid, length(Texts) + length(Users)) || Pid <- C2SPids],

    %[Pid ! resume_timeout || Pid <- C2SPids],

    %% User replaces each terminated session with a new one,
    %% rerouting unacked messages to the new session
    lists:foldl(fun(UserSpec, {PresCount, MsgCount}) ->
                        NewUser = connect_spec(UserSpec, session),
                        send_initial_presence(NewUser),
                        escalus:wait_for_stanzas(NewUser, PresCount + MsgCount),
                        timer:sleep(100),
                        escalus_assert:has_no_stanzas(NewUser),
                        escalus_connection:stop(NewUser),
                        {PresCount - 1, MsgCount * 2}
                end, {length(Users), length(Texts)}, UserSpecs),
    escalus_connection:stop(Bob).
...
relay_unacked_from_stopped_sessions(Config) ->
    ...
    FirstUser = connect_spec(hd(UserSpecs), sm_presence),
    ...
    RelayF = fun(NextSpec, CurUser) ->
                     escalus:wait_for_stanzas(CurUser, 10),
                     NewUser = connect_spec(NextSpec, sm_presence),
...
resend_unacked_to_different_res_after_resume_timeout(Config) ->
    ...
    %% alice comes back with different resource
    NewAlice = connect_spec([{resource, <<"2nd_resource">>} | AliceSpec], session),
    send_initial_presence(NewAlice),

    %% resume timeout passes
    timer:sleep(timer:seconds(?SHORT_TIMEOUT + 1)),

    %% alice receives unacked message and presence, as well as initial presence response
    %% the order of the messages may change, especially on CI, so we test all of them
    UnackedStanzas = escalus:wait_for_stanzas(NewAlice, 3),
